# [Import]
import random, os, time, pickle, inventory
...
class startupGame:
    ...
    def loadGame(self):  # I know this is completely pointless but it's quite fun
        os.system('cls'); files = ["maingame.py", "inventory.py"]
    ...
    def loadSaves(self): # TODO: Clean up and make this a sepparate file once there's too much stuff to save
        ...
        if os.stat(self.savePath).st_size == 0: 
            gamePlayer = player(50, 10, 4, 12, 20, 0); inventory.inventory().loadInv("resetInv"); self.saveInfo()  # If file empty, write base info into it
        else:  # If file has something, load it into the system
            data = pickle.load(open(self.savePath, "rb+"))
            gamePlayer = player(*data["Player"])
            if "Inventory" in data: inventory.inventory().loadInv(*data["Inventory"])
            else: inventory.inventory().loadInv("resetInv")
    def saveInfo(self): 
        p = gamePlayer
        data = {"Player": [p.hp, p.defns, p.atk, p.krit, p.kritChance, p.dist],
        "Inventory": [inventory.inventory.invList]}  # This thing took me a while to figure out
        pickle.dump(data, open(self.savePath, "wb"))

    def newGame(self): global gamePlayer; gamePlayer = player(50, 10, 4, 12, 20, 0); inventory.inventory().loadInv("resetInv"); self.saveInfo()
...
# [Set Enemy class]
class nme:
    def __init__(self, name, hp, defns, atk, lootID):
        self.name = name
        self.hp = hp
        self.defns = defns
        self.atk = atk
...
class battle:
    def battleInitiate(self):
        ...
        if monsterType < 20: setMonster = nme("Mutated Snake", 6, 4, 10, "nme-001-02-003")
        elif monsterType < 35: setMonster = nme("Mutated Wolf", 8, 12, 6, "nme-001-02-002")
        elif monsterType < 45: setMonster = nme("Mutated Zombie", 12, 5, 8, "nme-001-02-001")
        elif monsterType < 50: setMonster = nme("Baton Zombie", 6, 0, 8, "nme-001-01-003")
        elif monsterType < 75: setMonster = nme("Armored Zombie", 6, 8, 4, "nme-001-01-002")
        elif monsterType <= 100: setMonster = nme("Zombie", 6, 2, 4, "nme-001-01-001")
        # TODO: Put this and other indexes into one master index when too much stuff to keep track of
        ...
        while True:
            ...
            if action == "idatk":
                ...
                if setMonster.hp < 1: 
                    print("You win!\n")
                    inventory.inventory().lootStuff(setMonster.lootID)
                    break
...
class mainGame:
    def gameStart(self):
        while True:
            ...
            elif action == "idgoinv": inventory.invModeGo()
            elif action == "idexit":
